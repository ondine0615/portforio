...
class Kipris(object):
    def __init__(self, task_id=None):
        ...
        ###### 등록사항 api 정보 ###########
        # 0. 변동정보 -> 변동정보 operation를 통해 변동사항이 생긴 특허 등록번호를 받는다. 이 때 받은 번호가 권리정보, 권리자정보, 권리순위정보, 등록료정보 정보를 받기 위한 데이터가 된다. 
        self.list_services = [{
            # 다른 api를 추가하기 위해 list(dict()) 형태로 구성
    ...
    def data_union(self, type_):
        # 2개 이상의 변동사항 api를 사용할 때를 상정한다면 중복번호를 제거하고 하나의 리스트로 만드는 과정을 거칠 필요가 있음. 
        self.get_list(type_)
    ...
    def registration(self, number, error_date):
        try:
            ...
            if len(content)!=0:
                ...
                self.post_db(self.lastRightHolder_delete_query, number, self.connect_mysql)
